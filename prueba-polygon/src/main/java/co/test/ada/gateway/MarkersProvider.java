...
import co.test.ada.rest.client.request.RequestGeometriesContainer;
import co.test.ada.rest.client.response.Feature;
import co.test.ada.rest.client.response.Geometry;
import co.test.ada.rest.client.response.IStationsData;
import co.test.ada.rest.client.response.PosconsumoPoint;
import co.test.ada.rest.client.response.StationAgua;
import co.test.ada.rest.client.response.StationsDataAgua;
...
public class MarkersProvider {
	...
	private static final int VALUE_SR = 4326;//4218,4326,3857,3116
	...
	public List<CoordenadaDto> guardarCoordenadas(Feature feature) {
		...
		ring.forEach(parCoordenada->{
				...
				coordenadaModel.setLatitud(parCoordenada[1]);
				coordenadaModel.setLongitud(parCoordenada[0]);
				coordenadasModel.add(coordenadaModel);
			});
		return coordenadasModel;
	}
	
	public List<List<CoordenadaDto>> getInnerRings(Feature feature){
		feature.getGeometry().getRings().remove(0);
		List<List<double[]>> rings = feature.getGeometry().getRings();
		List<List<CoordenadaDto>> outerRings = new ArrayList<>();
		rings.forEach(ring->{
			outerRings.add(genCoordenadas(ring));
		});
		return outerRings;
	}
	
	public List<CoordenadaDto> genCoordenadas(List<double[]> ring){
		List<CoordenadaDto> coordenadasModel = new ArrayList<>();
		ring.forEach(parCoordenada->{
			CoordenadaDto coordenadaModel = new CoordenadaDto();
			coordenadaModel.setLatitud(parCoordenada[1]);
