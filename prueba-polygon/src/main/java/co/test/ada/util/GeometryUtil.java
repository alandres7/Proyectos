...
public final class GeometryUtil {
	...
	public static Polygon createPolygon(final List<CoordenadaDto> outerRing, final List<List<CoordenadaDto>> innersRings) {
	...
	public static List<List<LatLng>> getCoordenadasMaps(Polygon polygon){
		...
		while (i<polygon.getNumInteriorRing()) {
			coordenadasMaps.add(
					Arrays.asList(polygon.getInteriorRingN(i++).getCoordinates()).stream().filter(Objects::nonNull)
						.map(c->new LatLng(c.y, c.x)
								).collect(Collectors.toList())
						);
		}
